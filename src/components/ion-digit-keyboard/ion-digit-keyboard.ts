...
export class IonDigitKeyboard {
	...
	@Input()
	set leftActionOptions(v: boolean | ActionOptions) {
		if (typeof v == "object") {
		...
		if (typeof v == "boolean") {
			...
			if (v === true)
				this.log(
					'Left action button is set to "true", an empty button is displayed.'
				);
		}
	}
	get leftAction() {
	...
	@Input()
	set rightActionOptions(v: boolean | ActionOptions) {
		if (typeof v == "object") {
		...
		if (typeof v == "boolean") {
			...
			if (v === true)
				this.log(
					'Right action button is set to "true", an empty button is displayed.'
				);
		}
	}
	get rightAction() {
	...
	static show(callback: Function = () => {}): void {
		if (this.component && !this.component.visible) {
			...
			setTimeout(() => {
				callback();
				IonDigitKeyboard.onShow.next();
			}, this.getTransitionDuration(this.component.el.nativeElement));
		}
	}
	...
	private log(message: string, type: string = "log"): void {
		if (console) {
			...
			console.log(
				"%c◼︎ IonDigitKeyboard%c: " + message,
				"font-weight: bold; color: " + c + ";",
				""
			);
		}
	}
