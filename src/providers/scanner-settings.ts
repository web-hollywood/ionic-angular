...
export class ScannerSettings {
	...
	constructor(private events: Events, private enums: Enums) {
		this.maxScanningArea = {
			portrait: { width: 1, height: 1 },
	...
	public clampActiveScanningArea(
		...
		const getMargins: () => { portrait: Margins; landscape: Margins } = () => {
			...
			return {
				portrait: {
					top: isString(portraitConstraints.topMargin)
						? parseInt(portraitConstraints.topMargin as string, 10) /
							100 *
							screen.portrait.height
						: (portraitConstraints.topMargin as number),
					right: isString(portraitConstraints.rightMargin)
						? parseInt(portraitConstraints.rightMargin as string, 10) /
							100 *
							screen.portrait.width
						: (portraitConstraints.rightMargin as number),
					bottom: isString(portraitConstraints.bottomMargin)
						? parseInt(portraitConstraints.bottomMargin as string, 10) /
							100 *
							screen.portrait.height
						: (portraitConstraints.bottomMargin as number),
					left: isString(portraitConstraints.leftMargin)
						? parseInt(portraitConstraints.leftMargin as string, 10) /
							100 *
							screen.portrait.width
						: (portraitConstraints.leftMargin as number)
				},
				landscape: {
					top: isString(landscapeConstraints.topMargin)
						? parseInt(landscapeConstraints.topMargin as string, 10) /
							100 *
							screen.landscape.height
						: (landscapeConstraints.topMargin as number),
					right: isString(landscapeConstraints.rightMargin)
						? parseInt(landscapeConstraints.rightMargin as string, 10) /
							100 *
							screen.landscape.width
						: (landscapeConstraints.rightMargin as number),
					bottom: isString(landscapeConstraints.bottomMargin)
						? parseInt(landscapeConstraints.bottomMargin as string, 10) /
							100 *
							screen.landscape.height
						: (landscapeConstraints.bottomMargin as number),
					left: isString(landscapeConstraints.leftMargin)
						? parseInt(landscapeConstraints.leftMargin as string, 10) /
							100 *
							screen.landscape.width
						: (landscapeConstraints.leftMargin as number)
				}
			};
		};
		...
		if (!this.restricted) {
			...
			const portrait = {
				...
				width:
					(screen.portrait.width -
						margins.portrait.left -
						margins.portrait.right) /
					screen.portrait.width,
				height:
					(screen.portrait.height -
						margins.portrait.top -
						margins.portrait.bottom) /
					screen.portrait.height
			};
			portrait.x = (1 - portrait.width) / 2; // coordinates go from 0-1
			portrait.y = (1 - portrait.height) / 2; // coordinates go from 0-1
			let portraitArea = new Scandit.Rect(
				portrait.x,
				portrait.y,
				portrait.width,
				portrait.height
			...
			const landscape = {
				...
				width:
					(screen.landscape.width -
						margins.landscape.left -
						margins.landscape.right) /
					screen.landscape.width,
				height:
					(screen.landscape.height -
						margins.landscape.top -
						margins.landscape.bottom) /
					screen.landscape.height
			};
			landscape.x = (1 - landscape.width) / 2; // coordinates go from 0-1
			landscape.y = (1 - landscape.height) / 2; // coordinates go from 0-1
			let landscapeArea = new Scandit.Rect(
				landscape.x,
				landscape.y,
				landscape.width,
				landscape.height
			...
			if (
				(margins.portrait.top > 0 &&
					margins.portrait.right > 0 &&
					margins.portrait.bottom > 0 &&
					margins.portrait.right > 0) ||
				(margins.landscape.top > 0 &&
					margins.landscape.right > 0 &&
					margins.landscape.bottom > 0 &&
					margins.landscape.right > 0)
			) {
			...
			if (
				...
				this.settings.activeScanningAreaPortrait.height >=
					portraitArea.height ||
				this.settings.activeScanningAreaLandscape.width >=
					landscapeArea.width ||
